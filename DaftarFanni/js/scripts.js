function Checkinputs(e /*Area*/) { let flag = false; var el = $('#' + e).find("input[required] ,textarea[required] , select[required]"); el.map((x, element) => { if (element.value == '' || element.value == '-1') { RedAlert(element, ''); flag = true; } }); return flag; }
...
function ClearFields(e /*Area*/) { $("#" + e).find("input[type=text] , input[type=number]").val(""), $("#" + e).find("textarea").val(""), $("#" + e).find("select").prop("selectedIndex", 0) }
...
function AjaxData(a) { $.ajax({ type: "POST", url: a.url, data: JSON.stringify(a.param), contentType: "application/json;", dataType: "json", success: a.func, error: function (e) { console.log(e.responseJSON); RedAlert("n", e.statusText) } }) }

function FilterPartsByMachine(machine, part , func) {
  ...
  function fillelement(e) {
    ...
    var options = [];
    $('#' + part).empty().append($("<option></option>").attr("value", -1).text('انتخاب کنید'));
    data.map(x => {
      options.push('<option value="' + x.Id + '">' + x.UnitName + '</option>');
    })
    $('#' + part).append(options);
    $('#' + part).trigger('chosen:updated');
    if (func != undefined) {
      func();
    }
  }
