...
public partial class bastebandi_orderprior : System.Web.UI.Page
{
    ...
    {
        cnn.Open();
        var conf = new SqlCommand("update orders set conf = 1 where id = "+order_id.Value+" ",cnn);
        conf.ExecuteNonQuery();
        grid_orders.DataBind();
        pnl_order.Visible = true;
        pnl_order_detailes.Visible = false;
        ScriptManager.RegisterStartupScript(Page, GetType(), "script", "GreenAlert('n','تایید نهایی انجام شد');", true);
    }
}