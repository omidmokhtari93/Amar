...
public partial class _Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
            ...
            if ((((string)Session["level"] != "programer") || (Convert.ToInt32(Session["userid"]) != 15)) && (((string)Session["level"] != "mng_product") || (Convert.ToInt32(Session["userid"]) != 16)))
            {
    ...
    protected void btnshow_Click(object sender, EventArgs e)
    {
        ...
        cnn.Open();
        SqlCommand cmdweight = new SqlCommand("SELECT      ROUND(SUM(dbo.rikht_total.total * dbo.item.waight_raw), 2) AS weight FROM         dbo.rikht_total INNER JOIN dbo.item ON dbo.rikht_total.item_id = dbo.item.id WHERE     (dbo.rikht_total.tarikh BETWEEN '" + lbldate_s.Text + "' AND '" + lbldate_e.Text + "')", cnn);
        lbltotal_weight.Text = Convert.ToString(cmdweight.ExecuteScalar()).Replace('.', '/');
        cnn.Close();
       
        gridtotal.DataBind();
      
        gridtotal_day.DataBind();
    }
    ...
    protected void btnexcell_Click(object sender, EventArgs e)
    {
        ...
        Response.Charset = "";
        string FileName = "";
        FileName = "آمارریخته گری " + DateTime.Now + ".xls";
