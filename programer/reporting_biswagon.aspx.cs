...
public partial class _Default : System.Web.UI.Page
{
    ...
    protected void btnshow_Click(object sender, EventArgs e)
    {
        ...
        {
            SqlCommand cmd_wagon = new SqlCommand("SELECT     COUNT(num_wagon) AS Twagon FROM         (SELECT     MAX(tarikh) AS tarikh, num_wagon, SUM(waight) AS tonazh FROM bis_wagon WHERE     (tarikh = '" + lbldate_e.Text + "') GROUP BY num_wagon, sortt) AS bis_w", cnn);
            lblwagon.Text = Convert.ToString(cmd_wagon.ExecuteScalar());
        }

        cnn.Close();
        grid_raw.Caption = " آمار ضایعات خام ";
        grid_bis.Caption = " آمار ضایعات بیسکوییت ";
        grid_raw.DataBind();
        grid_bis.DataBind();
        gridbis.DataBind();
    ...
    protected void btnexcell_Click(object sender, EventArgs e)
    {
        ...
        Response.Charset = "";
        string FileName = "";
        FileName = "آمارکوره بیسکوییت " + DateTime.Now + ".xls";
